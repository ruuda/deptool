...
Post-generation checklist (run after writing code, before presenting):
 ...
 - For each ordering decision in user-visible structures (plan display, validation output, exhaustive destructure of `SystemDiff`-like aggregates): match the order in which actions execute in `activate()` (currently `symlinks → sysusers → units → quadlets`). New entries go where runtime order dictates; existing code that violates this need not be shuffled in unrelated changes.
