...
Post-generation checklist (run after writing code, before presenting):
 ...
 - For each new identifier (field, function, env var): `git grep` for analogous existing names and match the prevailing convention. Plural for collection directories (`sysusers_dir`, `quadlets_dir`); singular subject for action-printer functions (`write_unit_actions`, `write_quadlet_actions`); singular subject + `Changes` for change-aggregate types.
 - For each *internal* ordering decision (struct field order, destructures, parameter lists, owner-tracker declarations, config fields, managed-dir arrays): use alphabetical order by subject name (`quadlets`, `symlinks`, `sysusers`, `units`). Subsets drop the missing subjects but keep the relative positions. Apply consistently across every touchpoint in a change.
 - For *user-visible* output (plan display, executor logs, validation messages that list subjects): match the order in which actions execute in `activate()`. Users read the plan to predict execution; order has semantic meaning. Currently `activate()` runs in alphabetical-by-subject order so display matches the internal field order, but if a future constraint forces `activate()` to deviate, the display follows `activate()` (split the block if needed).
 - For each plan-display change: open `activate()`, list the per-action sequence, and verify the rendered plan matches it. If the new action's execution slot lies mid-way through an existing display block, split the block; do not append at the end.
 - For each change to the order or grouping of steps in `activate()` (merging phases, moving an action, changing what runs in which step): update `docs/deployment_phases.md`, which narrates the apply sequence and drifts silently otherwise. Leave a `TODO(ruuda)` marker rather than rewriting the prose.
 - For each doc comment or user-facing message: sweep for project names lowercase outside backticks (Podman, Git, Nix, Cargo, Linux, Deptool) AND for common nouns wrongly title-cased mid-sentence (quadlet, unit, service, app, host, deploy, manifest, commit, cluster). The rule applies to names introduced this session even if they are not in CLAUDE.md.
 - For each technical term in a doc comment or user-facing message: if a reader would need a glossary (subsumes, idempotent, monotonic, projection, total order, induced, dual), define it inline or rephrase in plain English.
 - For each user-visible change (new feature, new flag, new dir, new error, output format change): leave a `<!-- TODO(ruuda): ... -->` marker in `docs/changelog.md` under the unreleased section. The TODO marker is allowed without explicit permission; the prose is not.
